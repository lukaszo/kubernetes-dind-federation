...
function dind::deploy-externalipcontroller {
  ...
  dind::await_crd "ipclaims.ipcontroller.ext" 60
  dind::await_crd "ipclaimpools.ipcontroller.ext" 60
...
metadata:
    name: lb-pool
spec:
    cidr: ${EXTERNALIPCONTROLLER_RANGE}
EOF
  br="br-`docker network ls|grep ${CLUSTER_NAME}_default|awk '{print $1}'`"
  sudo ip addr add "${EXTERNALIPCONTROLLER_IP}" dev ${br}
}
...
elif [ $(basename "$0") = dind-deploy-externalipcontroller.sh ]; then
  source "${DIND_ROOT}/config.sh"
  dind::deploy-externalipcontroller
elif [ $(basename "$0") = dind-remove-externalipcontroller.sh ]; then
  source "${DIND_ROOT}/config.sh"
  dind::remove-externalipcontroller
fi
