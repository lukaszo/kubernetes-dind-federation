...
function dind::deploy-federation {
  ...
  docker run --rm -i -u `id -u $USER` --entrypoint /bin/bash -v "${certdir}:/certs" -w /certs cfssl/cfssl:latest -ec "$(cat <<EOF
    ...
    echo '{"CN":"CA","key":{"algo":"rsa","size":2048}}' | cfssl gencert -initca - | cfssljson -bare ca -
    echo '{"signing":{"default":{"expiry":"43800h","usages":["signing","key encipherment","server auth","client auth"]}}}' > ca-config.json
    echo '{"CN":"server","hosts":[""],"key":{"algo":"rsa","size":2048}}' | cfssl gencert -config=ca-config.json -ca=ca.pem -ca-key=ca-key.pem -hostname=127.0.0.1,coredns-etcd,coredns-etcd.${FEDERATION_NAMESPACE} - | cfssljson -bare "server"
...
EOF
  ...
  "cluster/kubectl.sh" create secret tls etcd-server -n "${FEDERATION_NAMESPACE}" --cert="${certdir}"/server.pem --key="${certdir}"/server-key.pem
  # install etcd
...
" 2>/dev/null)
  ...
  cat >${tmpfile} << EOF
    ...
    zones= ${DNS_ZONE}.
    etcd-cert-file = /etc/etcd/tls.crt
    etcd-key-file = /etc/etcd/tls.key
EOF

  "cluster/kubectl.sh" create namespace "${FEDERATION_NAMESPACE}-system"
  "cluster/kubectl.sh" create secret tls etcd-client -n "${FEDERATION_NAMESPACE}" --cert="${certdir}"/client.pem --key="${certdir}"/client-key.pem
